...
class RgbdFrame(object):
  ...
  def getNormals(s,renormalize=False,algo=None,path=None,reCompute=False):
    ...
    if s.n.size == 1:
      ...
      print "recomputing? {}".format(reCompute)
      normals = BufferedResult(head+'/',_getNormals,False)
      if 'n' in normals.get(inp,reCompute).keys():
        s.n = normals.get(inp,reCompute)['n']
      elif 'u_registered' in normals.get(inp,reCompute).keys():
        s.n = normals.get(inp,reCompute)['u_registered']
      else:
        print normals.get(inp,reCompute).keys()
    if renormalize:
...
#              print 'dp={}; dz={}'.format(dp,dz)
    ...
    fig.show()
    fig1.show()
  def saveNormals(s,path,renormalize=False,algo=None):
    n = s.getNormals(renormalize,algo)
    # save as tree rows and N cols in a cvs stile format
    np.savetxt(path,np.reshape(n,(n.shape[0]*n.shape[1],3)).T)
...
def _getNormals(inp):
  ...
  if algo == 'sobel' or algo == '':
    ...
    print 'matlab -nodesktop -nodisplay -r "addpath(genpath(\'~/workspace/research/vpCluster/matlab\')); compute_normals_from_file(\''+path+'_d.png\'); exit"'
    os.system('matlab -nodesktop -nodisplay -r "addpath(genpath(\'~/workspace/research/vpCluster/matlab\')); compute_normals_from_file(\''+path+'_d.png\'); exit"')
